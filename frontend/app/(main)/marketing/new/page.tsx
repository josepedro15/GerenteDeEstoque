...
import {
    ...
    getMarketingProductsByIds,
    generateCampaignWithGemini,
    saveCampaignForCurrentUser,
    type ProductCandidateMarketing,
...
export default function MarketingPage() {
    ...
    const autoGenerateFromChatRef = useRef(false);
    const loadedFromChatRef = useRef(false);

    // State
    const [step, setStep] = useState<'selection' | 'editor'>('selection');
    const [products, setProducts] = useState<ProductCandidateMarketing[]>([]);
    ...
    // Produtos vindos do chat (link "Gerar campanha"): preencher carrinho e iniciar produção
    useEffect(() => {
        const productIdsParam = searchParams.get('productIds');
        if (!productIdsParam?.trim() || loadedFromChatRef.current) return;
        const ids = productIdsParam.split(',').map((id) => id.trim()).filter(Boolean);
        if (ids.length === 0) return;
        loadedFromChatRef.current = true;
        (async () => {
            setLoading(true);
            try {
                const fromChat = await getMarketingProductsByIds(ids);
                if (fromChat.length > 0) {
                    setCart(fromChat);
                    setProducts((prev) => {
                        const existingIds = new Set(prev.map((p) => p.id));
                        const newOnes = fromChat.filter((p) => !existingIds.has(p.id));
                        return [...newOnes, ...prev];
                    });
                    autoGenerateFromChatRef.current = true;
                }
            } catch (e) {
                console.error(e);
            } finally {
            ...
            router.replace('/marketing/new', { scroll: false });
        })();
    }, [searchParams, router]);
    ...
    const [cart, setCart] = useState<ProductCandidateMarketing[]>([]);

    const handleAddToCart = (product: ProductCandidateMarketing) => {
    ...
    // Disparar geração automática quando o carrinho foi preenchido pelo link do chat
    useEffect(() => {
        if (autoGenerateFromChatRef.current && cart.length > 0) {
            autoGenerateFromChatRef.current = false;
            handleGenerate();
        }
    }, [cart]);
    ...
    const handleSend = async (finalStrategy: CampaignStrategy, visualUrl?: string) => {
        ...
        try {
            ...
            const saveResult = await saveCampaignForCurrentUser(
                finalStrategy,
                cart,
                visualUrl
            );
